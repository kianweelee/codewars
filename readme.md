...
def average_string(s):
    pattern = {"zero":0,"one":1, "two":2, "three":3, "four":4, "five":5, "six":6, "seven":7, "eight":8, "nine":9, 9:"nine", 8:'eight',7:"seven", 6:"six", 5:"five", 4:"four",3:"three", 2:'two', 1:'one',0:"zero"}
    if s:
        for x in s.split():
            if x not in pattern:
                return "n/a"
        return pattern[sum([pattern[x] for x in s.split()])//len(s.split())]
    return "n/a"
```
...
```python
def band_name_generator(name):
    return ["The {}".format(name.title()), name.title()+name[1:]][name[0] == name[-1]]
```
...
### 37. Equal Sides Of An Array

```python
def find_even_index(arr):
    for n in range(len(arr)):
        if sum(arr[:n]) == sum(arr[-1:n:-1]):
            return n
    return -1
```
...
### 35. Playing with digits

```python
def dig_pow(n, p):
    arr = [int(x) for x in list(str(n))]
    s = sum([n**m for m,n in enumerate(arr, p)])
    if s%n == 0:
        return s//n
    return -1
```
